...
class Board_write extends Component {
    ...
    Board_write = () =>{
        ...
        console.log("board에서 체크하는 토큰값:",token);
        //토큰값체크 
        JWTService.checkLogin(token);
        //Board 객체생성
        let Board = {
            title: this.state.title,
            content: this.state.content,
        }
        BoardApiService.boardWrite(token,Board)
        .then(res=>{
            console.log(res);
    ...
    render() {
        return (
            <div className="container">
                ...
                <button type="button" className="btn btn-primary" onClick={this.Board_write}>글쓰기</button>
            </div>
        );
    }
}
